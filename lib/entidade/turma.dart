...
class Turma {
  ...
  salvar(List<AlunoDTO> alunos, ITurma iTurma) {
    if (alunos.isEmpty)
      return "A turma deve ter pelo menos 1 aluno para ser cadastrada";
    ...
    if (idTurma == null) return "Ocorreu um erro as salvar a turma";
    turmaDTO.id = idTurma;
    String retorno = '';
    int idTurmaAluno;
    for (AlunoDTO aluno in alunos) {
      ...
      if (idTurma == null) {
        if (retorno.isEmpty) {
          retorno =
              "Ocorreu um erro ao vincular o(s) seguinte(s) aluno(s) a turma: ";
        }
